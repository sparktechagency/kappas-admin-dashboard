...
import { useDeleteTransactionMutation, useGetAllTransactionQuery } from '../../../features/transaction/transactionApi';
...
const TransactionTable = () => {
  ...
  const { data: allTransactionData, isLoading: transactionLoading, isError: transactionError } = useGetAllTransactionQuery({});
  ...
  const apiTransactions: Transaction[] = allTransactionData?.data?.payments || [];

  // Transform API data to include additional fields for display
  const transformedTransactions: Transaction[] = apiTransactions.map(transaction => ({
    ...transaction,
    ...
    customerEmail: `customer${transaction.transactionId.slice(-4)}@example.com`,
  ...
  const filteredTransactions = transformedTransactions.filter(transaction => {
    const matchesSearch =
      ...
      formatDate(transaction.createdAt).toLowerCase().includes(searchTerm.toLowerCase()) ||
      transaction.method.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (transaction.customerName && transaction.customerName.toLowerCase().includes(searchTerm.toLowerCase()));

    const matchesFilter =
      filterStatus === 'default' ||
      (filterStatus === 'paid' && transaction.status === 'Paid') ||
      (filterStatus === 'unpaid' && transaction.status === 'Unpaid');
  ...
  const handleView = (transactionId: string) => {
    const transaction = transformedTransactions.find(t => t.transactionId === transactionId);
    if (transaction) {
              ...
              {currentTransactions.length === 0 ? (
                ...
                currentTransactions.map((transaction) => (
                  <TableRow key={transaction._id} className="hover:bg-gray-50">
                    <TableCell className="text-gray-700">{formatDate(transaction.createdAt)}</TableCell>
                    <TableCell className="text-gray-700 font-mono text-sm" title={transaction.transactionId}>
                      {formatTransactionId(transaction.transactionId)}
                    </TableCell>
                    <TableCell className="text-gray-700">{formatPaymentMethod(transaction.method)}</TableCell>
                    <TableCell>
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-md text-sm font-medium ${transaction.status === 'Paid'
          ...
          {selectedTransaction && (
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <span className="col-span-3 text-sm text-gray-700">{formatDate(selectedTransaction.createdAt)}</span>
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <span className="text-sm font-medium text-gray-900">Transaction ID:</span>
                <span className="col-span-3 text-sm text-gray-700 font-mono">{formatTransactionId(selectedTransaction.transactionId)}</span>
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <span className="text-sm font-medium text-gray-900">Payment Method:</span>
                <span className="col-span-3 text-sm text-gray-700">{formatPaymentMethod(selectedTransaction.method)}</span>
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <span className="text-sm font-medium text-gray-900">Status:</span>
                <span className="col-span-3">
                  <span
                    className={`inline-flex items-center px-3 py-1 rounded-md text-sm font-medium ${selectedTransaction.status === 'Paid'
                      ? 'bg-green-600 text-white'
                      : 'bg-red-600 text-white'
                      }`}
                  >
              ...
              {selectedTransaction.amount && (
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <span className="col-span-3 text-sm text-gray-700">{selectedTransaction.amount}</span>
                </div>
              )}
              {selectedTransaction.customerName && (
                <div className="grid grid-cols-4 items-center gap-4">
                  <span className="text-sm font-medium text-gray-900">Customer Name:</span>
                  <span className="col-span-3 text-sm text-gray-700">{selectedTransaction.customerName}</span>
                </div>
              )}
              {selectedTransaction.customerEmail && (
                <div className="grid grid-cols-4 items-center gap-4">
                  <span className="text-sm font-medium text-gray-900">Customer Email:</span>
                  <span className="col-span-3 text-sm text-gray-700">{selectedTransaction.customerEmail}</span>
                </div>
              )}
              {selectedTransaction.description && (
                <div className="grid grid-cols-4 items-center gap-4">
                  <span className="text-sm font-medium text-gray-900">Description:</span>
                  <span className="col-span-3 text-sm text-gray-700">{selectedTransaction.description}</span>
                </div>
              )}
            </div>
          )}
        ...
        <AlertDialogContent className='w-full '>
          <AlertDialogHeader className='w-full'>
            ...
            <AlertDialogDescription className='w-full'>
              ...
              {selectedTransaction && ` with ID: ${formatTransactionId(selectedTransaction.transactionId)}`}.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
